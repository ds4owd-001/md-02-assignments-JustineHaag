...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    a barplot represents the number of occurences for each defined category, which is the same as (or the definition of) counting the occurences of each category in a column plot
