...
# code to bot display outliers from https://stackoverflow.com/questions/43499229/removing-outliers-from-boxplot-and-plotly

```

3.  What are the data types of the three variables used for aesthetic mappings?

    Continent is categorical (x), life expectancy is numerical (y), the filling colors are also categorical (?) althoughI would say colors are in the RAL sytem :D
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    A histogram is a special kind of bar plot, in which occurences are counted. In the case of the histogram, there is a counting of occurences of countries from each continent for which the life expectancy is between a value and this value plus 2 and a half years.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    You can change the time interval for each life expectancy class with the last break value. The two first values indicate where to start and where to stop (here 40 years for the minimum life expectancy and 85 for the maximum life expectancy recorded)
